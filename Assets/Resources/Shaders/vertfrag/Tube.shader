...
//        Tags { "RenderType"="Transparent" "Queue" = "Transparent" }
        ...
        Pass{
            ...
            float4 frag (varyings i) : COLOR {
                 ...
                 float3 fNorm = uvNormalMap( _NormalMap , i.pos ,  i.uv  * float2( 1. , 2.2), i.nor , 5.1 , 4.4);

                float3 fRefl = reflect( -i.eye , fNorm );
                float3 cubeCol = texCUBE(_CubeMap,fRefl ).rgb;

                float3 audio = tex2D( _AudioMap , float2( i.uv.y , 0)).xyz;
                return float4(cubeCol * float3( 1. , .6 , 0.1 ) , 1.);//.1 * float4(1,0.5f,0.0f,1.0) / ( i.dToPoint * i.dToPoint  * i.dToPoint );
            }
